...
def greetings(intent_request):
	return elicit(
		'Fulfilled',
		{
			...
			'content': 'Hi, I am ChatOps, a Chatbot. I am here to help you with DataLake related queries.'
		}
	)
...
def action_instances(intent_request):
	...
	words_stop = ['stop','shutdown','Power Off','Power Down']
	stack_list_dev = ['dev','Development','DEVELOPMENT']
	stack_list_non_prod = ['Non Prod','NON PROD','Non Production']
	...
	if instance_action in words_show:
	    ...
	    str1 = ' , \n'.join(instance_names)
	    print str1
	    output_message_action_instances = 'There are a total of  '+str(total_instances)+' '+str(instance_identifier)+' Instances in the '+str(stack_identifier)  + ' environment and they are as follows:-'+'\n'+str1
	...
	if instance_action in words_start:
	    ...
	    print('startAction')
	    output_message_action_instances = 'The '+str(instance_identifier)+' instance/s you have requested in the '+str(stack_identifier)  + ' environment has been '+str(instance_action)+'ed.'
...
def utilization_statistics(intent_request):
    ...
    response_describe = ec2_client.describe_instances(Filters=[{'Name': 'tag:Name','Values': ['*'+instance_identifier+'*']}])
    response_describe_new = ec2_client.describe_instances(Filters=[{'Name': 'tag:Name','Values': ['*'+stack_identifier+'*']}])
    print (type(stack_identifier))
    print stack_identifier
    print response_describe
    print response_describe_new
    words_stats = ['statistics','utilization','stats']
    stack_list_dev = ['dev','Development','DEVELOPMENT']
	stack_list_non_prod = ['Non Prod','NON PROD','Non Production']
	stack_list_prod = ['prod','PROD','Prod','Production','production']
    instance_ids = []
    ...
    for instance in instances:
		if instance_actions in words_stats:
			if (instance.id in instance_ids):
				...
				#message = 'The '+str(instance_identifier)+' instance/s you have requested in the '+str(stack_identifier)  + ' environment has been '+str(instance_action)+'ped.'
    return elicit(
		'Fulfilled',
		{
			'contentType': 'PlainText',
			'content': 'Here is the List of instances with Launch Time   Running Time    and   CPU Utilization: - ' + insts
        }
    )
...
def pricing_information(intent_request):
    ...
    for instance in instances:
        if (instance.id in instance_ids):
            ...
            insts = insts +" \n  \t "+instance.id + " ,  \t " + running_time_hr + " , \t " + instance_type + " , \t " +" $ "+str(price) + "\n" 
            print('Instance : ' + instance.id  + ', Running Time : ' + running_time_hr + ', Instance Type : ' + str(instance_type) + ', Price : ' + str(price))
    return elicit(
        'Fulfilled',
        {
            'contentType': 'PlainText',
            'content': 'Here is the List of instances with Total duration of the number of days Running Time ,  Instance Type   and Price: - ' + insts
        }
    )
	return elicit(
    		'Fulfilled',
...
# Price and Cost structure of machines based on U	ser Input
...
def pricing_information(intent_request):
    ...
    for instance in instances:
        if (instance.id in instance_ids):
            ...
            insts = insts +" \n  \t Instance ID : "+instance.id + " ,  \t Total running days and timestamp: " + running_time_hr + " hours , \t Instance Type: " + instance_type + " , \t Price: " +" $"+str(price) + "\n" 
            print('Instance : ' + instance.id  + ', Running Time : ' + running_time_hr + ', Instance Type : ' + str(instance_type) + ', Price : ' + str(price))
    ...
    return elicit(
		...
		{
			'contentType': 'PlainText',
			'content': 'Here is the following list of '+str(instance_identifier)+' Instances in the '+str(stack_identifier)+' environment with the cost statistics: - ' + insts
        }
    )
...
def status_list_services(intent_request):
	...
	#return check_service_list(instance_id)
	message = " Environment Current Status : \n Currently we have a total of "+str(service_list_count)+" \n services in the "+ str(stack_identifier)+" environment and \n out of which "+str(len(running_service_list))+" services are Healthy and "+str(len(stop_service_list))+" services are unhealthy!!!  \n The detailed list of services are : "+ str3
	return elicit(
			'Fulfilled',
    		{
    			'contentType': 'PlainText',
    			'content': message #'Here is list of services alongwith their State: ' + str3
            }
            )		
...
def dispatch(intent_request):
    ...
    if intent_name == 'action_instances':
		...
		{
			'contentType': 'PlainText',
			'content': 'Sorry!!  The request of '+ intent_name +' which you are looking for does not support with the current release '
					 
                      
        }
    ) 
	
	#raise Exception(' Intent with name ' + intent_name + 'is currently not support with the current release!')
...
def list_all_instances(intent_request):
	...
	return elicit(
		...
		{
			'contentType': 'PlainText',
			'content': 'Here is the folloiwng List of all the Instances in the Account.' + insts
        }
    )
...
def list_instance_tagged(intent_request):
	...
	return elicit(
		...
		{
			'contentType': 'PlainText',
			'content': 'Here is the following List of All the tagged Instances.'+insts
        }
    )
